...
import { Heart, MessageCircle, Share2, Image as ImageIcon, PartyPopper, Gift, Loader2 } from "lucide-react";
      ...
      <div className="lg:col-span-2">
        ...
        <div className="mt-6 rounded-card border border-surface-border bg-white p-4">
          ...
          <div className="mt-2 flex items-center justify-between">
            ...
            <button onClick={publish} disabled={posting || !draft.trim()} className="flex items-center gap-2 rounded-md bg-state-success px-4 py-2 text-sm font-medium text-white hover:opacity-90 disabled:opacity-60">
              ...
              <div key={post.id} className="rounded-card border border-surface-border bg-white p-4">
                ...
                <div className="mt-3 flex items-center gap-4 border-t border-surface-border pt-3">
                  <button
                    ...
                    className={clsx(
                      "flex items-center gap-1.5 text-sm transition-transform active:scale-125",
                      likedByMe ? "text-state-danger" : "text-state-danger/50 hover:text-state-danger"
                    )}
                  >
                    <Heart size={16} className={clsx("transition-transform hover:scale-110", likedByMe && "fill-state-danger")} />
                    {post.buzz_likes.length > 0 && post.buzz_likes.length}
                  </button>
                  <span className="flex items-center gap-1.5 text-sm text-ink-soft transition-colors hover:text-brand-700">
                    <MessageCircle size={16} /> {post.buzz_comments.length > 0 && post.buzz_comments.length}
                  </span>
                  <button className="flex items-center gap-1.5 text-sm text-ink-soft transition-all hover:scale-110 hover:text-brand-700"><Share2 size={16} /> Share</button>
                </div>
        ...
        <div className="rounded-card border border-surface-border bg-white p-5">
          ...
          <ul className="mt-3 space-y-2">
            {anniversaries.map((a) => {
              ...
              return (
                <li key={a.id} className="group flex items-center justify-between rounded-md px-2 py-1.5 text-sm transition-colors hover:bg-surface-subtle">
                  <span className="flex items-center gap-2">
                    <span className="flex h-7 w-7 items-center justify-center rounded-full bg-state-warningBg text-state-warning transition-transform group-hover:rotate-12 group-hover:scale-110">
                      <Gift size={13} />
                    </span>
                    <span className="text-ink">{a.full_name}</span>
                  </span>
                  <span className="flex items-center gap-2">
                    <span className="text-ink-muted">{new Date(a.date_joined).toLocaleDateString("en-GB", { day: "numeric", month: "short" })}</span>
                    <span className="rounded-full bg-brand-gradient px-2 py-0.5 text-xs font-medium text-white">{years}y</span>
                  </span>
                </li>
              );
            })}
